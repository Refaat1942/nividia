...
export default function BookingsPage() {
  ...
  const [form, setForm] = useState({ customer_id: '', room_id: '', booking_date: '', start_time: '', end_time: '', hours: '1', booking_status: 'confirmed', payment_status: 'pending', notes: '' });
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault();
    if (editing) {
      await api(`/bookings/${editing.id}`, { method: 'PATCH', body: JSON.stringify(form) });
      setEditing(null);
    } else {
          ...
          {editing && (
            <>
              <select className="input" value={form.booking_status} onChange={(e) => setForm({ ...form, booking_status: e.target.value })}>
                <option value="confirmed">مؤكد</option><option value="pending">معلق</option><option value="completed">مكتمل</option><option value="cancelled">ملغي</option>
              </select>
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="table-head">
            ...
            <th className="p-3 text-right">الساعات</th><th className="p-3 text-right">الحالة</th><th className="p-3 text-right">إجراءات</th>
          </tr></thead>
          <tbody>
            {bookings.map((b) => (
              <tr key={b.id} className="border-t">
                ...
                <td className="p-3 space-x-3 space-x-reverse">
                  <button onClick={() => { setEditing(b); setForm({ customer_id: b.customer_id, room_id: b.room_id, booking_date: b.booking_date, start_time: b.start_time?.slice?.(0,5) || b.start_time, end_time: b.end_time?.slice?.(0,5) || b.end_time, hours: String(b.hours), booking_status: b.booking_status, payment_status: b.payment_status, notes: b.notes || '' }); }} className="text-primary hover:underline">تعديل</button>
